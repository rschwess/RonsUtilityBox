...
snp_dict = {}   # intialise an empty dict
...
for key in snp_dict:
    ...
    # Delete Dict reads and alleic dict entries for memory release
    del snp_dict[key]['reads']
    # del snp_dict[key]['allelic_dict']
...
valid_tests_dict = {}
for key in snp_dict:
    # dont use uf insufficient reads, pvalue is not a valid number ref base is not the designated ref base or no alternative allele bases found
    if snp_dict[key]['flag_sufficient_reads'] and isNumber(snp_dict[key]['allelic_dict'][snp_dict[key]['alt']]['pvalue']) and snp_dict[key]['allelic_dict'][snp_dict[key]['alt']]['count'] > 0 and ref_allele_base == snp_dict[key]['ref']:
        valid_tests_dict[key] = snp_dict[key]['allelic_dict'][snp_dict[key]['alt']]['pvalue']

print(valid_tests_dict)

# 6.2) Perform FDR

# 6.3) Assign FDR values where appropriate (valid pvalues and ref and alt basess found)
...
for key in snp_dict:
    print( snp_dict[key]['chr'] + "\t" + str(snp_dict[key]['pos']) + "\t" + snp_dict[key]['id'] + "\t" + snp_dict[key]['report_ref'] + "\t" + snp_dict[key]['report_alt'] + "\t" + snp_dict[key]['report_other'] + "\t" + snp_dict[key]['report_comment'])
